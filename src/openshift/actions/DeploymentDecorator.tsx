import { k8sGet, K8sResourceCommon, K8sResourceKind, useK8sModel, useK8sWatchResource } from '@openshift-console/dynamic-plugin-sdk';
...
export const DeploymentDecorator: React.FC<DeploymentDecoratorProps> = ({
  ...
  if (element['resourceKind'] && element['resourceKind'] === 'apps~v1~Deployment') {
    ...
    if (labels['cryostat.io/name'] !== '' && labels['cryostat.io/namespace'] !== '') {
      // get the link to cryostat and use it in the href
      console.warn(resource);
      for(let i = 0; i < instances.length; i++) {
        if (instances[i].metadata?.name === labels['cryostat.io/name'] && instances[i].metadata?.namespace === labels['cryostat.io/namespace']) {
          k8sGet({
                model: routeModel,
      ...
      return (
        <a class="odc-decorator__link" href={`${routeUrl.current}/topology`} target="_blank" rel="noopener noreferrer" role="button" aria-label="Open Cryostat">
